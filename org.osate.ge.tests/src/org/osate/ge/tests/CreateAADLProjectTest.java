...
/*		bot.editorByTitle(projectName).show();
		...
		ICustomFeature[] cfs = shapeEditPart.getFeatureProvider().getCustomFeatures(new CustomContext(new PictogramElement[] { test.getFeatureProvider().getDiagramTypeProvider().getDiagram().getChildren().get(0) } ));
