...
public class PerformDiagramLayoutTest {
	...
	public void performDiagramLayout() {
		...
		final GraphicsAlgorithm ga = ((GraphitiShapeEditPart)editor.getEditPart(ElementNames.abstractTypeName).part()).getPictogramElement().getGraphicsAlgorithm();
		final int beforeX = ga.getX();
		final int beforeY = ga.getY();
		
		editor.clickContextMenu("Layout Diagram");

		final int afterX = ga.getX();
		final int afterY = ga.getY();
		assertTrue(beforeX != afterX || beforeY != afterY);
	}
}
